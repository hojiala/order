(function(root, factory) {
    ...
    function canonicalOrderId(value) {
        var id = text(value).replace(/[?#].*$/, '');
        if (!id) return '';
        if (id.indexOf('/') !== -1) id = id.split('/').pop();
        try { id = decodeURIComponent(id); } catch (_) {}
        id = id.replace(/\.json$/i, '').replace(/_繁體$/i, '');
        var previous = '';
    ...
    function orderIdentityKeys(order) {
        ...
        var original = order._integrationOriginal && typeof order._integrationOriginal === 'object'
            ? order._integrationOriginal
            : {};
        var sourcePath = text(
            (order.integration && order.integration.sourceRecordPath) ||
            order.sourceRecordPath ||
            (original.integration && original.integration.sourceRecordPath) ||
            original.sourceRecordPath
        );
        var source = text(order.source || original.source).toLowerCase();
        var phone = source === 'phone_ai' || source === 'pos_local' || /(?:^|\/)phone_ai\/orders\//i.test(sourcePath);
        var prefix = phone ? 'phone:' : 'order:';
        var values = [
            sourcePath,
            order.sourceOrderId,
            order.orderId,
        ...
        var seen = {};
        return values.map(canonicalOrderId).filter(function(id) {
            var key = prefix + id;
    ...
    function paymentRank(order) {
        var status = text(order && (order.paymentStatus || (order.payment && order.payment.status))).toLowerCase();
        var method = text(order && (order.paymentMethod || (order.payment && order.payment.method))).toLowerCase();
        if (status === 'refunded' || method === 'linepay_refund') return 5;
